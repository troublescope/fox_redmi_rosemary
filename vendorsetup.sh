...
if [ -f "$(gettop)/bootable/recovery/orangefox.cpp" ]; then
    ...
    if [ "$1" = "$FDEVICE" ] || [  "$FOX_BUILD_DEVICE" = "$FDEVICE" ]; then
        ...
        export FOX_VARIANT="A12-13_stable" 

        # Device info
        export FOX_AB_DEVICE=1
        export FOX_VIRTUAL_AB_DEVICE=1
        export TARGET_DEVICE_ALT="secret, maltose, rosemary_p"
        ...
        then
            ...
            then
                ...
                # Curl is supposed to be installed according to "Establishing a build environment" section in AOSP docs
